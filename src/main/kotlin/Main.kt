...
fun check(s:String): String {
    val st = mutableListOf<Char>() // Создаем стек со скобками
    val per = "(){}[]"
    for (i in s.indices) {
        ...
        //println(s[i])
        if (index >= 0) {
            if (index % 2 == 1) { // Закрывающая скобка
                ...
                if (last != per[index - 1]) {
                    return "NO" // Если последний элемент из стека не будет соотвествовать текущему, то расстановка неправильная
                }
            } else st.add(s[i])
        } else return "NO"
    }
